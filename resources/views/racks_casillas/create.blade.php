...
<form action="/admin/casillas" method="POST" id="frm_formulario" @submit="checkForm">
@csrf
    ...
    <div class="form-row">
        <div class="form-group col-md-12">
            ...
            <option value="">Seleccione un Rack</option>
                ...
                <option value="{{$item->rack_id}}">{{$item->rack_nombre}}</option>
                @endforeach
            </select>
        </div>
    </div>
  ...
  <div class="form-row">
    <div class="form-group col-md-12">
      ...
      <input type="text" class="form-control" v-model="nombre" style="text-transform:uppercase;" 
        onkeyup="javascript:this.value=this.value.toUpperCase();" name="nombre" id="nombre" placeholder="Nombre" value="{{old('nombre')}}">
    </div>
